...
class Riempi(MovingCameraScene):
    ...
    def construct(self):
        red_dice = Dice(6).set_color(PURE_RED).move_to(2 * LEFT)
        yellow_dice = Dice(3).set_color(YELLOW).move_to(3 * RIGHT + UP)
        blue_dice = Dice(5).set_color(PURE_BLUE).move_to(2 * RIGHT + 2*DOWN)
