...
class Scene(MovingCameraScene):
    ...
    def construct(self):
        ...
        quadrato_colorato = Square(fill_opacity=1, fill_color=RED, side_length=4)
        ...
        quadrato_colorato_interno = Polygon(quadrato_1.get_vertices()[1], quadrato_2.get_vertices()[2],
                                            ...
                                            fill_opacity=1, fill_color=GREEN, color=WHITE)
        ...
        self.play(Write(area_interna))
        self.play(FadeOut(quadrato_colorato))

        self.wait(delay)
        self.next_section()

        graffa = Brace(quadrato_colorato_interno, direction=DOWN + RIGHT)
        misura_lato = MathTex(r"l \overset{?}{\in} \mathbb{Q}").scale(scala_unita).move_to(graffa).shift(0.5 * DR).shift(0.7*RIGHT)
        self.play(Create(graffa))
        self.play(Write(misura_lato))
